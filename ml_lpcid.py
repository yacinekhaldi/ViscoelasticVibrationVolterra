...
ML_ROOT     = pathlib.Path("./results/ml_v8_lpcid").resolve()
...
def run_experiment():
    ...
    plt.plot(df_log["epoch"], df_log["val_core"], label="val_core", linestyle="--")
    plt.xlabel("epoch"); plt.ylabel("loss"); plt.legend(); plt.tight_layout()
    plt.savefig(ML_ROOT / "loss_curves__lpcid.png"); plt.close()
    ...
    plt.plot(y_true_all.ravel()[:sl], label="True", alpha=0.7)
    plt.plot(y_pred_all.ravel()[:sl], label="Pred", alpha=0.7)
    plt.legend(); plt.title("Forecast vs. Ground truth (slice) — LPCID")
    plt.tight_layout(); plt.savefig(ML_ROOT / "forecast_vs_true__lpcid.png"); plt.close()

    plt.figure(); plt.hist(residuals_all.ravel(), bins=50, alpha=0.85)
    plt.title("Physics residual distribution (raw) — LPCID")
    plt.tight_layout(); plt.savefig(ML_ROOT / "residual_hist__lpcid.png"); plt.close()

    plt.figure(); plt.hist(residuals_norm_all.ravel(), bins=50, alpha=0.85)
    plt.title("Physics residual distribution (normalized) — LPCID")
    plt.tight_layout(); plt.savefig(ML_ROOT / "residual_hist_norm__lpcid.png"); plt.close()
