...
End-to-end ML pipeline that:
  • loads synthetic viscoelastic vibration data created by your numerical pipeline,
  • builds train/val/test datasets,
  • trains a multi-task model with a novel physics layer (Neural Prony State-Space Layer, NPSSL),
  • evaluates forecasting, classification, and kernel-regression tasks,
  • exports all artifacts/arrays/plots for later re-analysis.

NOVEL IDEA (portable to other fields):
--------------------------------------
Neural Prony State-Space Layer (NPSSL):
  A differentiable memory layer that encodes the damping operator H[v] as a sum
  of stable exponential modes with positivity constraints. It uses internal
  states z_m[n] that update causally:
      z_m[n] = exp(-b_m dt) * z_m[n-1] + a_m * dt * v[n],   H[n] = sum_m z_m[n]
  • The decay rates {b_m} are placed on a fixed log-spaced grid (covering the
    time window of interest), while the amplitudes {a_m} are learned with
    softplus to preserve passivity (a_m >= 0).
  • This functions like a parametric, physically-plausible long-memory filter,
    analogous to learning a relaxation spectrum in viscoelasticity.
  • We combine NPSSL with data heads for (a) forecasting u, (b) classifying the
    kernel family, (c) regressing amplitudes, and include a physics residual loss.

Directory outputs (preserved from v1 and extended here):
--------------------------------------------------------
results/ml/
  ├─ config.json
  ├─ training_log.csv
  ...
  ├─ confusion_matrix.csv            # (NEW) numeric CSV export of CM
  ├─ spectrum_forcing.png
  ├─ metrics.json                    # (NEW) full metrics export (regression/forecast/residual)
  ├─ output.txt                      # (NEW) tee of console output
  ├─ checkpoints/
  │    └─ best_model.pt
  ├─ arrays/
  │    ├─ y_true_forecast.npy
  ...
  │    ├─ a_true.npy
  │    ├─ a_pred.npy
  │    ├─ K_reconstructed.npy   # (t_grid, K_pred)
  │    ├─ conf_matrix.npy       # (NEW) numeric CM
  │    └─ residuals.npy         # (NEW) residual samples (test)
  └─ samples/
       ├─ example_pred_<kname>.png
       ├─ K_curves_<kname>.png
       ├─ residual_histogram__<kname>.png      # (NEW) per-kernel residual distribution
       └─ residual_summary_bars.png            # (NEW) per-kernel residual mean±std bars

Notes:
------
• Hardcoded params for reproducibility. Adjust WINDOWS, HORIZON, etc. as needed.
• Requires: numpy, pandas, matplotlib, torch, scikit-learn.
• Assumes numerical results exist under results/kernels/<kernel>/<method>/timeseries.csv,
  and you’ve generated datasets/synthetic/manifest.json + forcing.npy via the earlier scripts.

v2 IMPROVEMENTS (surgical additions over v1, existing behavior preserved):
---------------------------------------------------------------------------
1) Confusion matrix also saved as npy/csv (kept from prior patch).
2) All stdout/stderr tee'd to results/ml/output.txt (kept).
3) Robust checkpointing (save-best on validation; guarded load).
4) Curriculum on physics residual: small at start, ramp up to target weight.
5) Residual smoothing to stabilize u'' in the residual term (less noise-amplification).
6) Exports: regression/forecast/residual metrics (JSON) + quick plots (kept and extended).
7) NEW: residuals per kernel family (histograms + summary bars).
"""
